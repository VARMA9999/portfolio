...
export const AnimatedWorkflowDiagram: React.FC<{ externalStep?: number; speed?: number }> = ({ externalStep, speed = 1 }) => {
  ...
  return (
    <div className="p-8 md:p-12 rounded-2xl border border-white/5 bg-[#050506] overflow-hidden relative min-h-[350px] flex flex-col items-center justify-center">
      <div className="flex items-center w-full max-w-4xl justify-between relative z-10">
        <motion.div className="flex flex-col items-center gap-3">
          <div className={`p-4 rounded-xl border ${step === 0 ? 'border-accent-primary bg-accent-primary/10 shadow-[0_0_20px_var(--accent-glow-subtle)]' : 'border-white/10 bg-white/5'}`}>
          ...
          <AnimatePresence>
            {step === 0 && (
              <motion.div
                ...
                className="absolute w-12 h-6 bg-accent-primary/20 border border-accent-primary rounded-full flex items-center justify-center -translate-y-1/2 -mt-3 text-[8px] font-mono text-accent-primary"
              >
                req
              </motion.div>
            )}
            {step === 1 && (
              <motion.div
                ...
                animate={{ left: '100%' }}
                transition={{ duration: 1, ease: 'linear' }}
                className="absolute w-20 h-6 bg-red-500/20 border border-red-500 rounded-full flex items-center justify-center -translate-y-1/2 -mt-3 text-[8px] font-mono text-red-500"
              >
                id=101 ❗
              </motion.div>
            )}
        ...
        <motion.div className="flex flex-col items-center gap-3">
          <div className={`p-4 rounded-xl border ${step === 2 ? 'border-red-500 bg-red-500/10 shadow-[0_0_20px_rgba(239,68,68,0.3)]' : 'border-white/10 bg-white/5'}`}>
          ...
          <AnimatePresence>
            ...
            {step === 3 && (
              <motion.div
                ...
                animate={{ right: '100%' }}
                transition={{ duration: 1, ease: 'linear' }}
                className="absolute w-16 h-6 bg-red-500/20 border border-red-500 rounded-full flex items-center justify-center -translate-y-1/2 -mt-3 text-[8px] font-mono text-red-500"
              >
                leak
              </motion.div>
            )}
        ...
        <motion.div className="flex flex-col items-center gap-3">
          <div className={`p-4 rounded-xl border ${step === 3 ? 'border-red-500 bg-red-500/10 shadow-[0_0_20px_rgba(239,68,68,0.3)]' : 'border-white/10 bg-white/5'}`}>
      ...
      <div className="mt-12 h-16 flex items-center justify-center text-center max-w-sm">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {step === 1 && <span className="text-red-400">2. Parameter Manipulation: Modifies ID in transit</span>}
            {step === 2 && <span className="text-red-500 font-bold">3. Missing Authorization: Server fails to validate ownership</span>}
            {step === 3 && <span className="text-red-500 border border-red-500/30 bg-red-500/10 px-3 py-1 rounded inline-block">4. Data Leakage: Unauthorized data returned</span>}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
...
export const AnimatedArchDiagram: React.FC = () => {
  return (
    <div className="p-8 rounded-2xl border border-white/5 bg-[#050506] overflow-hidden">
      <div className="flex flex-col items-center gap-4 relative">
        ...
        <motion.div 
          ...
          className="bg-white/5 border border-white/10 p-5 rounded-2xl w-full max-w-xl flex items-center gap-6 relative z-10 backdrop-blur-sm"
        >
          ...
          <div>
            ...
            <div className="font-mono text-[8px] text-text-muted uppercase mt-1">Presentation Layer</div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div>
            ...
            <div className="font-mono text-[8px] text-text-muted uppercase mt-1">Routing & Rate Limiting</div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="flex-1">
            ...
            <div className="font-mono text-[8px] text-red-400/70 uppercase mt-1">Failure Point: Missing Object Auth</div>
          </div>
        </motion.div>
        ...
        >
          ...
          <div>
            ...
            <div className="font-mono text-[8px] text-text-muted uppercase mt-1">Data Storage Layer</div>
          </div>
        </motion.div>
      </div>
    </div>
...
export const AnimatedRootCauseDiagram: React.FC<{ externalStep?: number }> = ({ externalStep }) => {
  const steps = [
    ...
    { label: 'Direct Database Query', icon: Search, color: 'text-red-400', desc: 'Query executes using untrusted input directly' },
    { label: 'Unauthorized Data Exposure', icon: Unlock, color: 'text-red-500', desc: 'Sensitive data returned to attacker' },
  ];

  return (
    <div className="p-8 md:p-12 rounded-2xl border border-white/5 bg-[#050506] overflow-hidden">
        ...
        {steps.map((st, i) => {
          ...
          return (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`p-4 rounded-xl border flex items-center gap-4 transition-all duration-300 ${isActive || i === steps.length - 1 && externalStep === undefined ? 'bg-red-500/10 border-red-500/30 shadow-[0_0_20px_rgba(239,68,68,0.2)]' : 'bg-white/5 border-white/10'}`}
            >
              <div className={`p-3 rounded-xl bg-white/5 ${isActive ? 'bg-red-500/20' : ''}`}>
