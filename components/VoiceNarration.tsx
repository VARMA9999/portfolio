...
export const VoiceNarration: React.FC<VoiceNarrationProps> = ({
  ...
  useEffect(() => {
    const prepareTemplates = async () => {
      ...
      try {
        const translated: any = { beginner: { ...base.beginner }, advanced: { ...base.advanced }, mentor: base.mentor, student: base.student };
        translated.beginner.modeIntro = await translateText(base.beginner.modeIntro, language);
        translated.advanced.modeIntro = await translateText(base.advanced.modeIntro, language);
        translated.beginner.transitions = {
          beginner: await translateText(base.beginner.transitions.beginner, language),
          attack: await translateText(base.beginner.transitions.attack, language),
          tech: await translateText(base.beginner.transitions.tech, language),
        };
        translated.advanced.transitions = {
          beginner: await translateText(base.advanced.transitions.beginner, language),
          attack: await translateText(base.advanced.transitions.attack, language),
          tech: await translateText(base.advanced.transitions.tech, language),
        };
        translated.beginner.summaryOpeners = await Promise.all(base.beginner.summaryOpeners.map((s: string) => translateText(s, language)));
        translated.beginner.explanationOpeners = await Promise.all(base.beginner.explanationOpeners.map((s: string) => translateText(s, language)));
        translated.beginner.attackOpeners = await Promise.all(base.beginner.attackOpeners.map((s: string) => translateText(s, language)));
        translated.beginner.impactOpeners = await Promise.all(base.beginner.impactOpeners.map((s: string) => translateText(s, language)));
        translated.beginner.preventionOpeners = await Promise.all(base.beginner.preventionOpeners.map((s: string) => translateText(s, language)));
        translated.beginner.conclusionOpeners = await Promise.all(base.beginner.conclusionOpeners.map((s: string) => translateText(s, language)));
        translated.beginner.fillers = await Promise.all(base.beginner.fillers.map((s: string) => translateText(s, language)));
  ...
  const queueChunk = useCallback((sIdx: number, cIdx: number) => {
    ...
    try {
      ...
      // CRITICAL FALLBACK: If browser has NO voices for this language code, 
      // fallback to the default voice (usually English) instead of staying silent.
      if (langVoices.length === 0) {
         langVoices = voices.filter(v => v.default) || [];
         if (langVoices.length === 0 && voices.length > 0) langVoices = [voices[0]];
      }

      const sorted = [...langVoices].sort((a, b) => {
        const aName = a.name.toLowerCase(); const bName = b.name.toLowerCase();
        const premiumKeywords = ["neural", "natural", "online", "google", "microsoft", "siri", "enhanced"];
        const aScore = premiumKeywords.reduce((acc, k) => acc + (aName.includes(k) ? 10 : 0), 0);
        const bScore = premiumKeywords.reduce((acc, k) => acc + (bName.includes(k) ? 10 : 0), 0);
        return bScore - aScore;
      });

      const genderKeywords = voiceGender === 'female' ? FEMALE_VOICE_KEYWORDS : MALE_VOICE_KEYWORDS;
      let genderFiltered = sorted.filter(v => genderKeywords.some(keyword => v.name.toLowerCase().includes(keyword)));
      if (genderFiltered.length === 0) genderFiltered = sorted;

      const voice = genderFiltered[0] || langVoices[0];
      if (voice) {
        utterance.voice = voice;
        utterance.lang = voice.lang; // STRICTURE: Always match utterance lang to selected voice's lang precisely to fix playback failures
      } else {
